 ...
 */
static void timer_subscribe(GMainLoop *loop, GSourceFunc func, gpointer udata)
{
	GSource	*gsrc;

 	LOG_DEBUG("timer_subscribe: report_int=%d", opt.o_report_int);
			 ...
			 const struct hsm_action_item *hai, const long hal_flags)
{
	...
	/* register a period timer callback for progress report */
	timer_subscribe(loop, cmd_progress_timer_cb, cb_args);
