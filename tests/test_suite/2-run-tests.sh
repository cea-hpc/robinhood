...
function db_schema_convert
{
    ...
    grep "Run 'robinhood --alter-db'" rh.log | grep -v "modification in ACCT_STAT primary key" && error "lismgr should deal with field shuffling"
    # no ALTER TABLE expected
    grep "ALTER TABLE" rh.log && error "no ALTER TABLE expected"
    ...
    grep "Run 'robinhood --alter-db'" rh.log | grep -v "modification in ACCT_STAT primary key" && error "report should deal with policy removal"
    [ "$DEBUG" = "1" ] && cat rh.log
    config_file=$(basename $cfg1) check_fcount $nbent
