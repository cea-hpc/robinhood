...
function test_sched_ratelim
{
    ...
    sum=$(grep "run summary" rh_migr.log | cut -d '|' -f 2 | cut -d ':' -f 2)
    time=$(echo $sum | cut -d ';' -f 1 | sed -e s/.*=// -e s/s$// -e s/^0//)
    # 5 entries to be migrated @1/sec => 5 sec run
    ((time==5)) || error "Unexpected migration run time: $time != 5"
...
run_test 236j  test_sched_ratelim test_ratelim.conf "Check action rate limitations"
