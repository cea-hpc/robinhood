...
static gboolean cmd_send_cb(GIOChannel *channel, GIOCondition cond, gpointer ud)
{
	...
	/* splice/sendfile API would be more elegant, but sometimes returned
	...
	if (nbytes == -1) {
		...
		LOG_DEBUG("cmd_send_cb: EOF at offset=%lu for "DFID,
			  args->offset, PFID(&args->hai->hai_fid));
		goto out_finished;
	} else {
...
out_finished:
	if (rc != 0) {
		LOG_ERROR(rc, "cmd_send_cb failed for "DFID,
...
static void cmd_termination_cb(GPid pid, gint status, gpointer ud)
{
	...
	if (WIFEXITED(status) && WEXITSTATUS(status) == 0) {
		...
		LOG_DEBUG("archive cmd failed (status=%d)", status);
		args->retcode = -ECHILD;
	}
