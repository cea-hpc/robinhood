...
(CFLAGS=-I/home/leibovi/export_rh_1101814/sherpa_cache/src/include $config_cmd && make -j $NB_PROC) > /dev/null || ( echo FAILED; ERRORS="$ERRORS Error using compilation switches:$config_cmd\n" )
...
if [[ $lustre = "--disable-fid-support" && $chglog = "--enable-changelogs" ]]; then echo "skipping conflicting switches: $config_cmd"; continue; fi
