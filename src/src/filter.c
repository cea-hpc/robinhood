...
static struct filter *
filter_compose(enum filter_operator op, int num, va_list args)
{
	struct filter *filter;
	...
	for (int i = 0; i < num; i++)
		filter->logical.filters[i] = va_arg(args, struct filter *);
...
filter_and(int num, ...)
{
	...
	va_start(args, num);
	filter = filter_compose(AND, num, args);
	va_end(args);

	return filter;
}

struct filter *filter_or(int num, ...)
{
	struct filter *filter;
	va_list args;

	va_start(args, num);
	filter = filter_compose(OR, num, args);
	va_end(args);

	return filter;
}

void __filter_negate(struct filter *filter)
{
	switch(filter->op) {
	case EQUAL:
		filter->op = NOT_EQUAL;
