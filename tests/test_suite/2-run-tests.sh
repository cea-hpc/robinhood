...
# Create a file and touch it to set atime/mtime. Also, check that
# crtime and ctime are properly set, using the search criteria of
# rbh-find, and display of rbh-report.
function posix_acmtime
{
    ...
    local db_acmtime=$($FIND $RH_ROOT/ -type f -f $RBH_CFG_DIR/$config_file -printf "%RAs %RMs %RCs")
    ...
    # for the time to change by at least one second, so ctime will be
    # updated
    sleep 5
...
function test_rbh_find_printf
{
    ...
    STR=$($FIND $RH_ROOT/ -type f -f $RBH_CFG_DIR/$config_file -printf "%RRF")
    ...
    STR=$($FIND $RH_ROOT/ -type f -f $RBH_CFG_DIR/$config_file -printf "%RCG")
    [[ $STR == "$(date +%G)" ]] || error "unexpected rbh-find result (209): $STR"

    STR=$($FIND $RH_ROOT/ -type f -f $RBH_CFG_DIR/$config_file -printf "QWERTY %RRc %RMA %RCA %RAp %RAT" 2>&1)
    [[ $STR == *"QWERTY"* ]] || error "unexpected rbh-find result (210): $STR"

    STR=$($FIND $RH_ROOT/ -type f -f $RBH_CFG_DIR/$config_file -printf "QWERTY %RROe %RMOS %RCOS %RAEx %RAEY" 2>&1)
    [[ $STR == *"QWERTY"* ]] || error "unexpected rbh-find result (211): $STR"

    STR=$($FIND $RH_ROOT/ -type f -f $RBH_CFG_DIR/$config_file -printf "QWERTY %RROA" 2>&1)
    [[ $STR == *"QWERTY %OA"* ]] || error "unexpected rbh-find result (212): $STR" # NB: invalid format

    STR=$($FIND $RH_ROOT/ -type f -f $RBH_CFG_DIR/$config_file -printf "QWERTY %RREB" 2>&1)
    [[ $STR == *"QWERTY %EB"* ]] || error "unexpected rbh-find result (213): $STR" # NB: invalid format

    STR=$($FIND $RH_ROOT/ -type f -f $RBH_CFG_DIR/$config_file -printf "%RR{%A, %B %dth, %Y %F}" 2>&1)
    [[ $STR == *"Error:"* ]] && error "unexpected rbh-find result (214): $STR"

    STR=$($FIND $RH_ROOT/ -type f -f $RBH_CFG_DIR/$config_file -printf "%RR{%A, " 2>&1)
