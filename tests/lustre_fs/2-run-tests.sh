...
function run_test
{
	...
	if [[ -z $only_test || "$only_test" = "$index" || "$only_test" = "$index_clean" ]]; then
		...
		if (( $DO_SKIP )); then
			...
			RC=$(($RC+1))
			if (( $junit )); then
				junit_report_failure "robinhood.$PURPOSE.Lustre.Test$index" "Test #$index: $args" "$dur" "ERROR" 
			fi
		else
			echo "TEST #$index : OK" >> $SUMMARY
			SUCCES=$(($SUCCES+1))
			if (( $junit )); then
				junit_report_success "robinhood.$PURPOSE.Lustre.Test$index" "Test #$index: $args" "$dur"
			fi
		fi
	fi
}
