...
class Status_hsmManager
{
    ...
    */
    public function getStat()
    {
        ...
        $size = array();
        $blks = array();
        $db_result = $this->db_request->select( null, ACCT_TABLE, array(STATUS_HSM), null );

        $stat = new Statistics();

        foreach( $db_result as $line )
        {
	    $stname = $line[STATUS_HSM]; 
    ...
    */
    ...
    {
        $db_result = $this->db_request->select( array( STATUS_HSM => $status ), ACCT_TABLE, null, $sort );
        return $db_result;
    }
