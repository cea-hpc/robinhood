...
struct backend_operations {
    int (*init)(const struct backend *backend, size_t size, va_list args);
 ...
 * @param size      the number of arguments to follow
 * @param ...       cf. the backend's documentation
 *
 * @return          0 on success, < 0 otherwise
 */
static inline int
backend_init(const struct backend *backend, size_t size, ...)
{
    va_list args;
    int rc;

    va_start(args, size);
    rc = backend->op->init(backend, size, args);
    va_end(args);
