...
function test_multirule_select
{
    ...
    grep "AS id FROM ENTRIES" $logfile |
      ...
      grep "OR ENTRIES.last_mod IS NULL" > /dev/null ||
...
function test_rmdir_depth
{
    ...
    grep "SELECT ENTRIES.id AS id FROM ENTRIES WHERE ENTRIES.type='dir'" $logfile > /dev/null ||
...
run_test 235   test_multirule_select test_multirule.conf "check sql query string in case of multiple rules"
run_test 236   test_rmdir_depth  test_rmdir_depth.conf "check sql query for rmdir with depth condition"
